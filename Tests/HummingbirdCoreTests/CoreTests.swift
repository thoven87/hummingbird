...
class HummingBirdCoreTests: XCTestCase {
    ...
    func testMaxConnections() async throws {
        ...
        try await testServer(
            responder: helloResponder,
            httpChannelSetup: .http1(),
            configuration: .init(address: .hostname(port: 0), availableConnectionHandler: .testMaximumAvailableConnections(10)),
            eventLoopGroup: Self.eventLoopGroup,
            logger: Logger(label: "Hummingbird")
        ) { port in
            try await withThrowingTaskGroup(of: Void.self) { group in
...
extension AvailableConnectionsHandlerFactory {
    static func testMaximumAvailableConnections(_ maxConnections: Int) -> Self {
        return .init {
            TestMaximumAvailableConnections(maxConnections)
        }
    }
}
